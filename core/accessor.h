...
class TensorAccessor : public TensorAccessorBase<T, N> {
    public:
        TensorAccessor(T *data, size_t *strides, size_t *dims) : TensorAccessorBase<T, N> (data, strides, dims) {}

        TensorAccessor<T, N-1> operator[](size_t i) {
            return TensorAccessor<T, N-1>(this->data_ + this->strides_[0] * i, this->strides_ + 1, this->dims_ + 1);
        }
};
